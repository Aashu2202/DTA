"""
Company Knowledge Base for D-Table Analytics
This module contains all structured company information used by the AI assistant.
Keep this centralized for easy updates to company details.
"""

COMPANY_INFO = """
==================================
D-TABLE ANALYTICS - COMPANY PROFILE
==================================

COMPANY NAME:
D-Table Analytics

VISION:
To empower businesses with data-driven decision making and workflow automation solutions,
enabling them to transform raw data into actionable insights and automate critical business processes.

MISSION:
Providing comprehensive data management, business intelligence, and communication automation solutions
to help organizations achieve operational excellence and competitive advantage.

---

DETAILED SERVICES:

1. DATA MANAGEMENT SOLUTIONS
   • Data collection, storage, and organization
   • Database design and optimization
   • ETL (Extract, Transform, Load) processes
   • Data quality assurance and validation
   • Real-time data synchronization
   • Secure data backup and disaster recovery

2. MIS (Management Information Systems)
   • Custom dashboard development and analytics
   • Real-time reporting and KPI tracking
   • Business intelligence and data visualization
   • Predictive analytics and forecasting
   • Executive decision support systems
   • Performance monitoring and alerts

3. WHATSAPP AUTOMATION
   • Automated customer communication workflows
   • Order and transaction notifications
   • Customer support automation
   • Marketing campaign automation
   • Two-way messaging and chatbot integration
   • Analytics and engagement metrics

---

TOOLS & TECHNOLOGIES:

Backend & Data:
   • Python with FastAPI (High-performance async APIs)
   • MongoDB (Flexible document database)
   • PostgreSQL (Robust relational database)
   • Redis (Caching and session management)

Frontend & UI:
   • React.js (Modern interactive interfaces)
   • TypeScript (Type-safe JavaScript)
   • Material-UI & Tailwind CSS (Design frameworks)

AI & Automation:
   • OpenAI GPT-4 (Advanced language model)
   • Natural Language Processing (NLP)
   • Machine Learning algorithms

DevOps & Infrastructure:
   • Docker & Kubernetes (Containerization)
   • AWS / Cloud deployment
   • CI/CD pipelines
   • REST APIs & Webhooks

---

CONTACT INFORMATION:

Email: contact@dtable-analytics.com
Phone: +1-800-DTABLE-1 (1-800-382-2531)
Website: https://www.dtable-analytics.com
Support Portal: https://support.dtable-analytics.com

Business Hours:
   • Monday - Friday:10:00 AM - 6:00 PM (IST)
   • Saturday: 10:00 AM -6:00 PM (IST)
   • Sunday: Closed
   • Holidays: Company holidays apply

Office Address:
   1st Floor, Kolar Castle, 101, 
   Chuna Bhatti Rd, Chuna Bhatti, 
   Bhopal, Madhya Pradesh 462039

---

ADDITIONAL INFO:

Why Choose D-Table Analytics?
   ✓ Team of experienced data engineers and analysts
   ✓ Proven track record with 100+ successful projects
   ✓ 24/7 customer support and technical assistance
   ✓ Custom solutions tailored to your business needs
   ✓ Competitive pricing and flexible engagement models
   ✓ Certified professionals and industry best practices
   ✓ Secure, scalable, and reliable infrastructure

Industries Served:
   • E-commerce & Retail
   • Healthcare & Pharma
   • Finance & Banking
   • Manufacturing
   • Hospitality
   • Education
   • Government & Public Sector
"""
